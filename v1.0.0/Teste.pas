...
type
  TForm1 = class(TForm)
    ...
    pEmojisView: TPanel;
    ScrollEmojis: TScrollBox;
    EmojisCollection: TImageCollection;
    OpenDlg: TOpenDialog;
    pStickersControlesView: TPanel;
    pSearchView: TPanel;
    Edit1: TEdit;
    pGifsView: TPanel;
    ...
    pRecentesView: TPanel;
    Label23: TLabel;
    vtImg0: TVirtualImage;
    vtImg1: TVirtualImage;
    vtImg2: TVirtualImage;
    vtImg3: TVirtualImage;
    vtImg4: TVirtualImage;
    vtImg5: TVirtualImage;
    vtImg6: TVirtualImage;
    pAnimaisView: TPanel;
    Label21: TLabel;
    ...
    procedure MyEmojiMouseEnter(Sender: TObject);
    procedure MyEmojiMouseLeave(Sender: TObject);
    ...
    procedure pButton1StickersClick(Sender: TObject);
    procedure pButton0EmojisClick(Sender: TObject);
    procedure pEmojisMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    procedure pTextBoxMouseEnter(Sender: TObject);
    procedure FormClick(Sender: TObject);
    procedure memChatMSGClick(Sender: TObject);
    procedure VirtualImage42Click(Sender: TObject);
    procedure VirtualImage43Click(Sender: TObject);
    procedure VirtualImage21Click(Sender: TObject);
    procedure VirtualImage49Click(Sender: TObject);
    procedure VirtualImage50Click(Sender: TObject);
    procedure VirtualImage56Click(Sender: TObject);
    procedure VirtualImage57Click(Sender: TObject);
    procedure VirtualImage15Click(Sender: TObject);
    procedure FormPaint(Sender: TObject);
    procedure memChatMSGMouseEnter(Sender: TObject);
  private
    ...
    procedure SetPanel(AIndex: Integer = 0);
  ...
  if Key = VK_RETURN then
    ...
    Begin
      ...
      begin
        ...
        XO.O['input_message_content'].O['text'] := SO;
        XO.O['input_message_content'].O['text'].S['@type'] := 'formattedText';
        XO.O['input_message_content'].O['text'].S['text'] := txtMsg2.Text;
...
procedure TForm1.VirtualImage15Click(Sender: TObject);
begin
  ScrollEmojis.VertScrollBar.Position := 100;
  VirtualImage15.ImageIndex := 10;

  VirtualImage42.ImageIndex := 13;
  VirtualImage43.ImageIndex := 14;
  VirtualImage21.ImageIndex := 23;
  VirtualImage49.ImageIndex := 19;
  VirtualImage50.ImageIndex := 3;
  VirtualImage56.ImageIndex := 5;
  VirtualImage57.ImageIndex := 15;
  //VirtualImage15.ImageIndex := 11;
end;

procedure TForm1.VirtualImage21Click(Sender: TObject);
begin
  if pRecentesView.Visible then
    ScrollEmojis.VertScrollBar.Position := 0;

  VirtualImage21.ImageIndex := 22;
...
procedure TForm1.VirtualImage3MouseEnter(Sender: TObject);
begin
  (Sender as TVirtualImage).Width  := (Sender as TVirtualImage).Width + 10;
  (Sender as TVirtualImage).Height := (Sender as TVirtualImage).Height + 10;
  (Sender as TVirtualImage).Left   := (Sender as TVirtualImage).Left - 5;
  (Sender as TVirtualImage).Top    := (Sender as TVirtualImage).Top - 5;
end;

procedure TForm1.VirtualImage3MouseLeave(Sender: TObject);
begin
  (Sender as TVirtualImage).Width  := (Sender as TVirtualImage).Width - 10;
  (Sender as TVirtualImage).Height := (Sender as TVirtualImage).Height - 10;
  (Sender as TVirtualImage).Left   := (Sender as TVirtualImage).Left + 5;
...
procedure TForm1.VirtualImage42Click(Sender: TObject);
begin
  if pRecentesView.Visible then
    ScrollEmojis.VertScrollBar.Position := 66
  else
    ScrollEmojis.VertScrollBar.Position := 0;

  VirtualImage42.ImageIndex := 12;
...
procedure TForm1.VirtualImage43Click(Sender: TObject);
begin
  ScrollEmojis.VertScrollBar.Position := 469;
  VirtualImage43.ImageIndex := 14;
...
procedure TForm1.VirtualImage49Click(Sender: TObject);
begin
  ScrollEmojis.VertScrollBar.Position := 300;
  VirtualImage49.ImageIndex := 18;
...
procedure TForm1.VirtualImage50Click(Sender: TObject);
begin
  ScrollEmojis.VertScrollBar.Position := 400;
  VirtualImage50.ImageIndex := 2;
...
procedure TForm1.VirtualImage56Click(Sender: TObject);
begin
  ScrollEmojis.VertScrollBar.Position := 500;
  VirtualImage56.ImageIndex := 4;
...
procedure TForm1.VirtualImage57Click(Sender: TObject);
begin
  ScrollEmojis.VertScrollBar.Position := 200;
  VirtualImage57.ImageIndex := 15;
...
procedure TForm1.MyEmojiMouseEnter(Sender: TObject);
begin
  (Sender as TVirtualImage).Width  := (Sender as TVirtualImage).Width + 10;
  (Sender as TVirtualImage).Height := (Sender as TVirtualImage).Height + 10;
  (Sender as TVirtualImage).Left   := (Sender as TVirtualImage).Left - 5;
...
procedure TForm1.MyEmojiMouseLeave(Sender: TObject);
begin
  (Sender as TVirtualImage).Width  := (Sender as TVirtualImage).Width - 10;
  (Sender as TVirtualImage).Height := (Sender as TVirtualImage).Height - 10;
  (Sender as TVirtualImage).Left   := (Sender as TVirtualImage).Left + 5;
...
begin
  ...
  c := TDirect2DCanvas.Create(AEdit.Handle, ClientRect);
  c.BeginDraw;
  try
...
procedure TForm1.imgSendClick(Sender: TObject);
var
  XO: ISuperObject;
begin
...
function ExtractEmojiName(AVirtualImage: TVirtualImage): String;
var
  I: Integer;
  NameStr: String;
Begin  //Pessoas\😂
  for I := 0 to Length(AVirtualImage.ImageName) do
  Begin
    if AVirtualImage.ImageName.IndexOf('\') > I then
    Begin
      NameStr := NameStr + AVirtualImage.ImageName.Chars[I];
    End;
  End;
...
begin
//*
  txtMSG2.Text := txtMSG2.Text +
  //ExtractEmojiName((Sender as TVirtualImage));
  (Sender as TVirtualImage).ImageName;
  ShowColorEmojis(txtMSG2);

  if (Sender as TVirtualImage).Parent.Name <> 'pRecentesView' then
  Begin
    case Count of
      0:Begin
        vtImg0.ImageIndex := (Sender as TVirtualImage).ImageIndex;
        vtImg0.Visible := True;
      End;
      1:Begin
        vtImg1.ImageIndex := (Sender as TVirtualImage).ImageIndex;
        vtImg1.Visible := True;
      End;
      2:Begin
        vtImg2.ImageIndex := (Sender as TVirtualImage).ImageIndex;
        vtImg2.Visible := True;
      End;
      3:Begin
        vtImg3.ImageIndex := (Sender as TVirtualImage).ImageIndex;
        vtImg3.Visible := True;
      End;
      4:Begin
        vtImg4.ImageIndex := (Sender as TVirtualImage).ImageIndex;
        vtImg4.Visible := True;
      End;
      5:Begin
        vtImg5.ImageIndex := (Sender as TVirtualImage).ImageIndex;
        vtImg5.Visible := True;
      End;
      6:Begin
        vtImg6.ImageIndex := (Sender as TVirtualImage).ImageIndex;
        vtImg6.Visible := True;
      End;
    end;

    if Count < 7 then //Ate 6
      Count := Count + 1;
    if Count = 7 then
      Count := 0;

    pRecentesView.Visible := True;
...
procedure TForm1.SetPanel(AIndex: Integer);
Begin
  ...
  case AIndex of
    ...
    1:Begin //pStickersView
      pIndicadorSTICKERS.Color := clHighlight;
      pStickersView.Visible := True;
      pStickersView.BringToFront;
    End;

    2:Begin //pStickersView
      pIndicadorGIFS.Color := clHighlight;
      pGifsView.Visible := True;
      pGifsView.BringToFront;
    End;
...
procedure TForm1.memChatMSGClick(Sender: TObject);
begin
 // sleep(ATimeEfectCardinal);
  pEmojis.Visible := False;
  memChatMSG.SetFocus;
...
procedure TForm1.memChatMSGMouseEnter(Sender: TObject);
begin
  memChatMSG.SetFocus;
end;

procedure TForm1.pButton0EmojisClick(Sender: TObject);
begin
  SetPanel(0); //EMOJIS
end;

procedure TForm1.pButton1StickersClick(Sender: TObject);
begin
  SetPanel(1); //StickersView
end;
...
begin
  if Sender is TInjectChatControl then
  Begin
    sleep(ATimeEfectCardinal);
    pEmojis.Visible := False;
  End;
...
procedure TForm1.pEmojisMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
    pEmojis.Visible := True;
end;

procedure TForm1.pTextBoxMouseEnter(Sender: TObject);
begin
  sleep(ATimeEfectCardinal);
  pEmojis.Visible := False;
end;
